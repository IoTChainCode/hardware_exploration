...
void osal_run_system( void )
{
  ...
  osalTimeUpdate();
#endif
  Hal_ProcessPoll();//polling uart/spi/hid
#endif /* USE_ICALL */
  ...
  if (idx < tasksCnt)//find events
  {
    uint16 events;
    halIntState_t intState;

    HAL_ENTER_CRITICAL_SECTION(intState);//close interrupt
    events = tasksEvents[idx];//read each task's events
    tasksEvents[idx] = 0;//Clear the Events for this task.
    HAL_EXIT_CRITICAL_SECTION(intState);//open interrupt
    //
    activeTaskID = idx;
    events = (tasksArr[idx])( idx, events );//task process.idx-task id,events-task's event
