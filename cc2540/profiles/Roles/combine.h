...
#define GAPROLE_STATE               0x31A  //!< Reading this parameter will return GAP Combine Role State. Read Only. Size is uint8.
...
#ifndef GAPROLE_NUM_RSSI_LINKS
#define GAPROLE_NUM_RSSI_LINKS     4
#endif
...
//GAP Combine Role States.
typedef enum
{
  GAPROLE_INIT = 0,                       //!< Waiting to be started
  GAPROLE_STARTED,                        //!< Started but not advertising
  GAPROLE_ADVERTISING,                    //!< Currently Advertising(advertising)
  GAPROLE_ADVERTISING_NONCONN,            //!< Currently using non-connectable Advertising
  GAPROLE_WAITING,                        //!< Device is started but not advertising, is in waiting period before advertising again(disconnect)
  ...
  GAPROLE_CONNECTED,                      //!< In a connection(connect)
 ...
 */
//Profile Callbacks
typedef void (*gapRolesParamUpdateCB_t)( uint16 connInterval,uint16 connSlaveLatency,uint16 connTimeout );//Callback when the connection parameteres are updated.
typedef void (*gapRolesStateNotify_t)( gaprole_States_t newState );//Callback when the device has been started.  Callback event to the Notify of a state change.
typedef void (*gapRolesRssiRead_t)( int8 newRSSI );//Callback when the device has read an new RSSI value during a connection.
typedef void (*pfngapRoleRssiCB_t)( uint16 connHandle,int8  rssi);//RSSI Read Callback Function
typedef uint8 (*pfngapRoleEventCB_t)( gapRoleEvent_t *pEvent);//Central Event Callback Function:TRUE if safe to deallocate event message, FALSE otherwise.
...
typedef struct
{
    ...
    //central
    pfngapRoleRssiCB_t   rssiCB;   //!< RSSI callback.
    pfngapRoleEventCB_t  eventCB;  //!< Event callback.
}gapRoleCB_t;
...
/**
 * @defgroup GAPROLES_PERIPHERAL_API GAP Combine Role API Functions
 *
 * @{
 */
 ...
 */
extern bStatus_t GAPRole_StartDevice( gapRoleCB_t *pAppCallbacks );
 ...
 */
extern bStatus_t GAPRole_TerminateConnection( uint16 handle );
 ...
 */
extern bStatus_t GAPRole_EstablishLink( uint8 highDutyCycle, uint8 whiteList,
 ...
 */
extern bStatus_t GAPRole_StartDiscovery( uint8 mode, uint8 activeScan, uint8 whiteList );
 ...
 */
extern bStatus_t GAPRole_StartRssi( uint16 connHandle, uint16 period );
 ...
 *          bleIncorrectMode: No link.<BR>
 */
extern bStatus_t GAPRole_CancelRssi(uint16 connHandle );
 ...
 * @return      void
 */
extern void GAPRole_Init( uint8 task_id,uint8 profile);
