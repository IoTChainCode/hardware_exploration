...
//private
void sha256_transform(uint8 *block)//64byte
{
	uint32 i;
...
void sha256_final(uint8 *result)
{
	...
	word2byte(ctx.buffer+SHA256_BLOCK-8,ctx.count[1]);
	mem_swap8(ctx.buffer+SHA256_BLOCK-8,4);
	word2byte(ctx.buffer+SHA256_BLOCK-4,ctx.count[0]);
	...
	for (i=0;i<32/4;i++)
	{
		word2byte(&result[i<<2],ctx.state[i]);
